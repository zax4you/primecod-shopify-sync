// api/sync-orders-with-storage.js - Enhanced sync that stores results for dashboard
export default async function handler(req, res) {
  ...
  try {
    console.log('🚀 Starting Enhanced Sync with Result Storage...');
    const startTime = new Date();
    const polandTime = startTime.toLocaleString("en-US", {timeZone: "Europe/Warsaw"});
    ...
    const totalMatched = syncResults.orders_updated + syncResults.matching_stats.already_processed;
    const matchRate = Math.round((totalMatched / syncResults.matching_stats.total_attempts) * 100);

    // 🎯 STORE SYNC RESULTS FOR DASHBOARD
    const dashboardData = {
      timestamp: endTime.toISOString(),
      poland_time: polandTime,
      success: true,
      duration: `${duration}s`,
      orders_processed: syncResults.orders_updated,
      total_matched: totalMatched,
      match_rate: matchRate,
      errors: syncResults.errors.length,
      type: 'enhanced_sync',
      summary: {
        total_leads: syncResults.total_leads_processed,
        new_updates: syncResults.orders_updated,
        already_processed: syncResults.matching_stats.already_processed,
        fulfilled: syncResults.fulfilled_orders,
        paid: syncResults.paid_orders,
        refunded: syncResults.refunded_orders,
        processing: syncResults.processing_orders
      }
    };

    // Store in environment variable-like structure (simple approach)
    // In a real app, you'd use a database, but for Vercel, we'll use response headers
    console.log('📊 Storing sync results for dashboard:', dashboardData);
    ...
    // Set custom headers for dashboard to read
    res.setHeader('X-Last-Sync-Time', polandTime);
    res.setHeader('X-Last-Sync-Duration', `${duration}s`);
    res.setHeader('X-Last-Sync-Orders', syncResults.orders_updated.toString());
    res.setHeader('X-Last-Sync-Match-Rate', matchRate.toString());
    res.setHeader('X-Last-Sync-Errors', syncResults.errors.length.toString());
    ...
    // Store error data for dashboard
    const errorTime = new Date().toLocaleString("en-US", {timeZone: "Europe/Warsaw"});
    res.setHeader('X-Last-Sync-Time', errorTime);
    res.setHeader('X-Last-Sync-Duration', 'Error');
    res.setHeader('X-Last-Sync-Orders', '0');
    res.setHeader('X-Last-Sync-Match-Rate', '0');
    res.setHeader('X-Last-Sync-Errors', '1');
...
// Enhanced matching logic (copy from your working enhanced script)
async function processLeadWithEnhancedMatching(lead, shopifyStore, shopifyAccessToken) {
  ...
  // Method 3: Partial email matching
  ...
  // Method 4: Fuzzy email matching
  if (!shopifyOrder && lead.email) {
    shopifyOrder = await findShopifyOrderByFuzzyEmail(lead.email, shopifyStore, shopifyAccessToken);
    if (shopifyOrder) {
  ...
  if (shopifyOrder.tags && shopifyOrder.tags.includes('primecod-')) {
    return {
      primecod_reference: lead.reference,
  ...
  // Process the order (same logic as your working sync)
  const updates = [];
...
// Helper functions (copy from your working sync)
...
async function findShopifyOrderByFuzzyEmail(email, shopifyStore, shopifyAccessToken) {
  ...
  for (const variation of domainVariations) {
    ...
    const match = await findShopifyOrderByEmail(testEmail, shopifyStore, shopifyAccessToken);
...
// Include all your working helper functions (fulfillOrderWithTracking, markOrderAsPaid, etc.)
...
async function markOrderAsPaid(orderId, shopifyStore, shopifyAccessToken) {
  try {
    ...
    const data = await response.json();
    return !!data.data?.orderMarkAsPaid?.order?.id;
